...
import static de.adito.aditoweb.vaadin.addons.gantt.client.SvgUtil.setAttributeNS;
...
import de.adito.aditoweb.vaadin.addons.gantt.client.shared.GanttUtil;
...
public class SvgArrowWidget extends Widget implements ArrowElement {
    ...
    protected PointerMoveHandler pointerMoveHandler = new PointerMoveHandler() {
        ...
        public void onPointerMove(PointerMoveEvent event) {
            ...
            if (!(pointerDownPoint.getX() == GanttUtil.getTouchOrMouseClientX(event
                    .getNativeEvent()) && pointerDownPoint.getY() == GanttUtil.getTouchOrMouseClientY(event
                    .getNativeEvent()))) {
                pointerTouchStartedTimer.cancel();
                handleMove(event.getNativeEvent());
            }
    ...
    public SvgArrowWidget() {
        Element predecessorArrow = SvgUtil.createSVGElementNS("svg");
        ...
        Element g = SvgUtil.createSVGElementNS("g");
        SvgUtil.setAttributeNS(g, "stroke", "black");
        SvgUtil.setAttributeNS(g, "stroke-width", "1");
        curve = SvgUtil.createSVGElementNS("path");
        addStyleName(curve, "curve-line");
        SvgUtil.setAttributeNS(curve, "fill", "none");
        startingPoint = SvgUtil.createSVGElementNS("circle");
        startingPoint.getStyle().setProperty("pointerEvents", "visiblePainted");
        addStyleName(startingPoint, "start-p");
        SvgUtil.setAttributeNS(startingPoint, "stroke-width", "2");
        SvgUtil.setAttributeNS(startingPoint, "r", "7");
        SvgUtil.setAttributeNS(startingPoint, "fill", "black");
        endingPoint = SvgUtil.createSVGElementNS("circle");
        endingPoint.getStyle().setProperty("pointerEvents", "visiblePainted");
        addStyleName(endingPoint, "end-p");
        SvgUtil.setAttributeNS(endingPoint, "stroke-width", "2");
        SvgUtil.setAttributeNS(endingPoint, "r", "5");
        SvgUtil.setAttributeNS(endingPoint, "fill", "black");
    ...
    public void setWidth(double width) {
        ...
        SvgUtil.setAttributeNS(getElement(), "width", getWidthWithMargin());
    }

    @Override
    public void setHeight(double height) {
        this.height = (int) height;
        SvgUtil.setAttributeNS(getElement(), "height", getHeightWithMargin());
    }
    ...
    protected void internalDraw(ArrowPositionData d) {
        ...
        SvgUtil.setAttributeNS(startingPoint, "cx", d.calcStartPointX() + getMargin());
        SvgUtil.setAttributeNS(startingPoint, "cy", d.calcStartPointY() + getMargin());

        int endPointX = d.calcEndPointX();
        int endPointY = d.calcEndPointY();
        SvgUtil.setAttributeNS(endingPoint, "cx", endPointX + getMargin());
        SvgUtil.setAttributeNS(endingPoint, "cy", endPointY + getMargin());
    }
    ...
    protected void internalDrawCurve(ArrowPositionData d) {
        ...
        SvgUtil.setAttributeNS(curve, "d", s.toString());
    }
    ...
    protected void startMoving(NativeEvent event, Element element) {
        ...
        capturePoint = new Point(GanttUtil.getTouchOrMouseClientX(event),
    ...
    protected void handleMove(NativeEvent event) {
        Point movePoint = new Point(GanttUtil.getTouchOrMouseClientX(event),
                GanttUtil.getTouchOrMouseClientY(event));
