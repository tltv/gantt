...
public class StepWidget extends AbstractStepWidget {
    ...
    private ArrowElement.ArrowChangeHandler arrowChangeHandler = new ArrowElement.ArrowChangeHandler() {
    ...
    {
        //run createPredecessorElemets() because if the Step is now connected to a Group-Step, the connection gets not drawn.
        //Stop Afterwards is there is either no predecessor or the predecessor is a Group-Step. We want no visible connection between those two
        createPredecessorElements();

        if (predecessorStepWidget == null || predecessorStepWidget.getStep().isGroup())
            return;
    ...
    {
        //If there is no predecessor or the predecessor is a group remove the arrow. Between regular Steps and Group-Steps
        //should be no visible connection
        if (predecessorStepWidget == null || predecessorStepWidget.getStep().isGroup()) {
