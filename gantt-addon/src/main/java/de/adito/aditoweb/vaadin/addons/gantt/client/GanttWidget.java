...
public class GanttWidget extends ComplexPanel implements HasEnabled, HasWidgets {
    ...
    private PointerDownHandler msPointerDownHandler = new PointerDownHandler() {
        ...
        public void onPointerDown(PointerDownEvent event) {
            ...
            capturePoint = new Point(GanttUtil.getTouchOrMouseClientX(event.getNativeEvent()),
    ...
    private PointerMoveHandler msPointerMoveHandler = new PointerMoveHandler() {
        ...
        public void onPointerMove(PointerMoveEvent event) {
            ...
            movePoint = new Point(GanttUtil.getTouchOrMouseClientX(event.getNativeEvent()),
                    GanttUtil.getTouchOrMouseClientY(event.getNativeEvent()));
    ...
    protected void onTouchOrMouseDown(NativeEvent event) {
        ...
        capturePoint = new Point(GanttUtil.getTouchOrMouseClientX(event), GanttUtil.getTouchOrMouseClientY(event));
        movePoint = new Point(GanttUtil.getTouchOrMouseClientX(event), GanttUtil.getTouchOrMouseClientY(event));
    ...
    protected void onTouchOrMouseUp(NativeEvent event) {
        ...
        if (bar == targetBarElement && isClickOnNextMouseup()) {
            ...
            if (resizing) {
                ...
                if (moveInProgress) {
                    moveCompleted(bar, GanttUtil.getTouchOrMouseClientY(event), event);
                } else {
                    resetBarPosition(bar);
                }
            }
    ...
    protected boolean onTouchOrMouseMove(NativeEvent event) {
        ...
        double deltax = GanttUtil.getTouchOrMouseClientX(event) - capturePoint.getX();
        double deltay = GanttUtil.getTouchOrMouseClientY(event) - capturePoint.getY();
    ...
    private void updateContainerStyle() {
        ...
        if (resDivElementCount > 2) {
            ...
            secondResolutionBlockWidth = GanttUtil.getBoundingClientRectWidth(secondResolutionBlock);
        }
    ...
    private void updateContainerBackgroundSize(boolean contentOverflowingHorizontally, double gridBlockWidthPx) {
        ...
        if (contentOverflowingHorizontally || useAlwaysPxSizeInBackground()) {
            ...
            double contentWidth = GanttUtil.getBoundingClientRectWidth(content);
    ...
    private int getElementHeightWithMargin(Element div) {
        ...
        marginHeight = GanttUtil.getMarginByComputedStyle(div);
    ...
    private double getContentWidth() {
        return GanttUtil.getBoundingClientRectWidth(content);
    }
